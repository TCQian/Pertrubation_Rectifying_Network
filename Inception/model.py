...
class Model(object):
  ...
  def _build_optim(self):
    def prn_minimize(loss, step, var_list):
      if self.config.optimizer == "sgd":
        self.sgd_learning_rate = tf.train.exponential_decay(self.learning_rate, step,
                                           1000, 0.1, staircase=True)
        optim = tf.train.GradientDescentOptimizer(self.sgd_learning_rate)
        #optim = tf.train.GradientDescentOptimizer(self.learning_rate)
      elif self.config.optimizer == "adam":
        self.adam_learning_rate = tf.train.exponential_decay(self.learning_rate, step,
                                           1000, 0.1, staircase=True)
        optim = tf.train.AdamOptimizer(self.adam_learning_rate)
    ...
    self.prn_summary = tf.summary.merge([
        ...
        tf.summary.scalar("fooling_rate", self.fooling_rate),
        tf.summary.scalar("benchmark_accuracy", self.benchmark_accuracy),
        tf.summary.scalar("adam_learning_rate", self.adam_learning_rate),
    ])
  ...
  def _build_prn(self, layer):
    with tf.variable_scope("PRN") as sc:
      layer = conv2d(layer, 64, 3, 1, activation_fn=None, scope="conv_0")	# TODO
      layer = repeat(layer, self.resnet_num, resnet_block, scope="resnet")

      #layer = conv2d(layer, 3, 1, 1, activation_fn=None, scope="conv_1")	# TODO
      #output = tf.nn.relu(layer, name="relu_output")
      #output = tanh(layer, name="tanh_output")

      layer = conv2d(layer, 16, 1, 1, activation_fn=None, scope="conv_11")	# TODO

      output = conv2d(layer, 3, 1, 1, activation_fn=None, scope="conv_1")	# TODO
...
#        tf.summary.scalar("build_loss/prn_loss",
#                          tf.reduce_mean(self.prn_loss)),
#        #tf.summary.histogram('P_y', self.P_y_print),
#        tf.summary.scalar("prn_accuracy", self.prn_accuracy),
#        tf.summary.scalar("fooling_rate", self.fooling_rate),
#        tf.summary.scalar("benchmark_accuracy", self.benchmark_accuracy),
#        #tf.summary.scalar("sgd_learning_rate", self.sgd_learning_rate),
#    ])
